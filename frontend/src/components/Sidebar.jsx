...
export default function Sidebar({ activeTab, setActiveTab, unreadAlerts = 3, user, onLogout }) {
  ...
  return (
    <aside
      ...
      className={`glass-sidebar flex flex-col shrink-0 h-screen max-h-screen select-none shadow-[6px_0_30px_rgba(0,0,0,0.5)] relative z-30 transition-all duration-300 ease-in-out ${
        isExpanded ? 'w-76' : 'w-20'
      }`}
      style={{ willChange: 'width' }}
    >
      {/* 1. Top Brand Header (Clickable -> Returns to Dashboard/Home) */}
      <div className="p-3.5 border-b border-emerald-500/15 relative overflow-hidden bg-gradient-to-b from-emerald-950/40 via-emerald-950/20 to-transparent shrink-0">
        {/* Ambient background glow */}
        <div className="absolute -top-10 -left-10 w-28 h-28 bg-emerald-500/20 rounded-full blur-2xl pointer-events-none"></div>

        <div className="flex items-center justify-between relative z-10">
          {/* Clickable Brand / Logo Link */}
          <div
            onClick={() => setActiveTab('dashboard')}
            className="flex items-center gap-3 cursor-pointer group flex-1 min-w-0"
            title="Go to Farmer Dashboard / Home"
          >
            <div className="relative shrink-0">
              <div className="h-11 w-11 rounded-2xl bg-gradient-to-br from-emerald-400 via-teal-500 to-emerald-700 text-slate-950 flex items-center justify-center shadow-lg shadow-emerald-500/30 border border-emerald-300/40 transition-all duration-300 group-hover:scale-105 group-hover:shadow-emerald-400/50">
                <Sprout className="h-6 w-6 text-slate-950 stroke-[2.2] animate-float-slow" />
              </div>
              <span className="absolute -bottom-1 -right-1 flex h-3.5 w-3.5">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-3.5 w-3.5 bg-emerald-400 border-2 border-[#070c16]"></span>
              </span>
            </div>

            {/* Revealed on Expand */}
            {isExpanded && (
              <div className="min-w-0 flex-1 transition-opacity duration-200 animate-in fade-in">
                <div className="flex items-center justify-between">
                  <span className="text-lg font-black tracking-tight text-white flex items-center gap-1.5 font-display group-hover:text-emerald-300 transition-colors">
                    KisanKendra
                  </span>
          ...
          {isExpanded && (
            ...
            >
              {isPinned ? <Pin size={14} className="text-emerald-400" /> : <PinOff size={14} />}
            </button>
          )}
        </div>
      </div>

      {/* 2. Navigation Scroll Body */}
      <div className="flex-1 overflow-y-auto overflow-x-hidden px-2.5 py-4 space-y-6">
        {/* Section 1: Mandi Operations */}
        <div>
          {isExpanded ? (
            <div className="px-2.5 pb-2 flex items-center justify-between">
              ...
              <span className="text-[9px] font-bold text-slate-500 font-mono">07 HUBS</span>
            </div>
          ) : (
            ...
            {mainNav.map((item) => {
              ...
              return (
                <button
                  ...
                  } rounded-xl text-xs font-bold transition-all duration-200 cursor-pointer overflow-hidden ${
                    isActive
                      ? 'bg-gradient-to-r from-emerald-900/90 to-emerald-950 text-white border border-emerald-400/50 shadow-[0_0_20px_rgba(16,185,129,0.3)] before:absolute before:left-0 before:top-1.5 before:bottom-1.5 before:w-1 before:bg-emerald-400 before:rounded-r-full'
                      : 'text-slate-400 hover:text-white hover:bg-emerald-950/40 hover:border-emerald-500/20 border border-transparent'
                  }`}
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <Icon
                      ...
                      className={`shrink-0 transition-all duration-200 ${
                        isActive
                          ? 'text-emerald-300 scale-110 drop-shadow-[0_0_8px_rgba(52,211,153,0.7)]'
                  ...
                  {isExpanded && item.isLive && (
                    <span className="flex items-center gap-1 text-[9px] font-black text-emerald-400 bg-emerald-950/80 border border-emerald-500/40 px-2 py-0.5 rounded-full">
        ...
        {/* Section 2: Intelligence & Weather */}
        <div>
          {isExpanded ? (
            <div className="px-2.5 pb-2 flex items-center justify-between">
              ...
              <span className="text-[9px] font-bold text-cyan-400/80 bg-cyan-950/60 px-1.5 py-0.5 rounded border border-cyan-500/30">
                AI ENGINE
              </span>
            </div>
          ) : (
              ...
              return (
                <button
                  ...
                  className={`w-full group relative flex items-center ${
                    ...
                    isActive
                      ? 'bg-gradient-to-r from-teal-900/90 to-emerald-950 text-white border border-cyan-400/50 shadow-[0_0_20px_rgba(6,182,212,0.3)] before:absolute before:left-0 before:top-1.5 before:bottom-1.5 before:w-1 before:bg-cyan-400 before:rounded-r-full'
                      : 'text-slate-400 hover:text-white hover:bg-emerald-950/40 hover:border-emerald-500/20 border border-transparent'
                  }`}
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <Icon
                      ...
                      className={`shrink-0 transition-all duration-200 ${
                        isActive
                          ? 'text-cyan-300 scale-110 drop-shadow-[0_0_8px_rgba(34,211,238,0.7)]'
          ...
          {/* Interactive Live Weather Telemetry (Shown when expanded) */}
          {isExpanded && (
            <div
              onClick={() => setActiveTab('weather')}
              className="mt-3.5 p-3 rounded-2xl bg-gradient-to-br from-emerald-950/80 via-[#071912] to-teal-950/60 border border-emerald-500/30 hover:border-emerald-400/60 shadow-lg transition-all duration-200 cursor-pointer group relative overflow-hidden animate-in fade-in"
            >
          ...
          {/* Real-time Yard Telemetry Flow Bar (Shown when expanded) */}
          {isExpanded && (
            <div className="mt-2.5 p-3 rounded-2xl bg-slate-900/80 border border-slate-800/90 shadow-sm relative overflow-hidden animate-in fade-in">
        ...
        <div>
          ...
          {/* 24x7 Mandi Helpline SOS Card (Shown when expanded) */}
          {isExpanded && (
            <div className="mt-3 p-3 rounded-2xl bg-gradient-to-br from-amber-950/50 to-orange-950/30 border border-amber-500/30 relative overflow-hidden animate-in fade-in">
      ...
      {/* 3. User Profile & System Status Footer */}
      <div className="p-3 border-t border-emerald-500/15 bg-black/40 backdrop-blur-md shrink-0">
        <div className={`flex items-center ${isExpanded ? 'justify-between gap-3' : 'justify-center'}`}>
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="relative shrink-0">
              <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-emerald-500 to-teal-500 text-slate-950 flex items-center justify-center font-bold text-xs shadow-md border border-emerald-300/40">
                {user?.name?.charAt(0) || 'K'}
              </div>
              <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-emerald-400 border-2 border-[#070c16]"></span>
            </div>
          ...
          {isExpanded && onLogout && (
            <button
              type="button"
              onClick={onLogout}
              className="p-2 rounded-xl text-slate-400 hover:text-rose-300 hover:bg-rose-950/40 hover:border-rose-500/30 border border-transparent transition-all cursor-pointer shrink-0"
              title="Sign Out"
            >
              <LogOut size={15} />
            </button>
        ...
        {isExpanded && (
          <div className="mt-2 pt-2 border-t border-emerald-500/10 flex items-center justify-between text-[10px] text-slate-500 animate-in fade-in">
